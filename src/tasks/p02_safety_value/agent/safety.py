...
class Safety(Agent):
    ...
    def _compute_target(self, next_states: torch.Tensor, g_values: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
    ...
    def update(self, batch) -> Dict[str, float]:
        ...
        states, next_states, g_values, _ = batch
        ...
        for epoch in range(self.learning_epochs):
            ...
            target_values = self._compute_target(next_states, g_values)
    ...
    def evaluate(self, data_loader, threshold: float = 0.0) -> Dict[str, float]:
        ...
        for states, next_states, g_values, future_max_g in data_loader:
            states = states.to(self.device, non_blocking=True)
            next_states = next_states.to(self.device, non_blocking=True)
            g_values = g_values.to(self.device, non_blocking=True)
            ...
            # Diagnostic: collect only empirical risky samples
            tensors = {
                "g": g_values,
                "future": future_max_g,
